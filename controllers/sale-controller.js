...
const getNextRecieptNo = async () => {
  try {
    const highestReceipt = await Sale.findOne()
      .sort("-reciept_no")
      .select("reciept_no")
      .lean();
...
const addSale = async (req, res, next) => {
  ...
  const saleSchema = Joi.object({
    customerId: reqStr,
    branch: reqNum,
    date: reqDate.default(() => Math.floor(Date.now() / 1000)),
  ...
  try {
    ...
    if (!newSale) return createError(res, 400, "Unable to add new Sale!");
    return successMessage(res, newSale, "Sale Successfully Created!");
  } catch (err) {
...
const GetAllSale = async (req, res, next) => {
  let allSales;
  try {
    allSales = await Sale.find().populate("sales");
    if (!allSales) return createError(res, 404, "Sales record not found!");
...
const GetBranchSales = async (req, res, next) => {
  ...
  let allSales;
  try {
    allSales = await Sale.find({ branch }).populate("sales");
    if (!allSales)
      return createError(res, 404, "Sales record not found for branch!");
...
const UpdateSales = async (req, res, next) => {
  ...
  let sale;
  try {
    sale = await Sale.findById(salesId);
    if (!Sale) {
      return createError(res, 404, "Sale with such id was not found!");
    }
    ...
    await sale.save();
    return successMessage(res, item, "Sales Successfully Updated!");
  } catch (err) {
...
const DeleteSale = async (req, res, next) => {
  ...
  try {
    ...
    if (!DeleteSale)
      return createError(res, 400, "Such Sale with saleId does not exist!");
    ...
    return successMessage(
      ...
      `Sale ${DeleteSale._id} and associated sales are successfully deleted!`
    );
  } catch (err) {
    console.log("error: ", err);
    return createError(res, 500, err.message || err);
